module.exports = function (redis) {
            ...
            if not hasJoined then
              ...
              if canJoin then
                ...
                if event.startedAt then
                  local msg = { type='start', event=event }
                  redis.call('publish', 'events/' .. event.id, cjson.encode(msg))
                end
  ...
  redis.defineCommand('rejoinEvent', {
    numberOfKeys: 3,
    lua: `
      local eventKey = KEYS[1]
      local pendingKey = KEYS[2]
      ...
      if not event.startedAt then
        return redis.error_reply('not started yet')
      end
  ...
  redis.defineCommand('leaveEvent', {
    numberOfKeys: 3,
    lua: `
      local eventKey = KEYS[1]
      local pendingKey = KEYS[2]
      ...
      local event = cjson.decode(eventJSON)
      local tableIndex = false
      local usersJoinedCount = 0

      for i, v in pairs(event.userIds) do
        usersJoinedCount = usersJoinedCount + 1
        if v == userId then
          tableIndex = i
        end
      ...
      if tableIndex == false then
        return redis.error_reply('not in event')
      end

      table.remove(event.userIds, tableIndex)
      table.remove(event.aliases, tableIndex)

      eventJSON = cjson.encode(event)
      redis.call('set', eventKey, eventJSON)

      local msg = { type='leave', userId=userId }
      redis.call('publish', 'events/' .. event.id, cjson.encode(msg))

      if usersJoinedCount - 1 == 0 then
        redis.call('srem', activeKey, eventKey)
        redis.call('del', eventKey)

        local msg = { type='finish', event=event }
        redis.call('publish', 'events/' .. event.id, cjson.encode(msg))
      end
    `
  })
